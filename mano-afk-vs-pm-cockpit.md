...
| # | Finding | 严重度 | mano-afk 发现？ | mano-afk 已修复？ | Moss 22 条覆盖？ | design-spec 生效后可覆盖？ |
|---|---------|--------|------------|-------------|------------|----------------------|
| P1 | 已完成订单可编辑所有字段 | Medium | ✅（U3） | ✅ 已修复 | ❌ 未覆盖 | ✅ D3 终态无操作必测 |
| P2 | 状态推进无确认步骤，误点不可逆 | Medium | ✅（U2） | ✅ 已修复 | ❌ 未覆盖 | ✅ D3 回退验证必测 |
| P3 | 删除商品无引用保护 | Medium | ✅（U5） | ✅ 已修复 | ❌ 未覆盖 | ⚠️ Backlog（跨实体依赖测试待补入） |
| P4 | 删除客户无引用保护 | Medium | 未检查 | — | ❌ 未覆盖 | ⚠️ Backlog（跨实体依赖测试待补入） |
| P5 | 删除确认弹窗不显示具体对象 | Low | ✅（U1） | ✅ 已修复 | ❌ 未覆盖 | ❌ 未覆盖（属 UX 细节，非测试设计范畴） |
